...
public class ViewMyAuctions extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	static String templateDir = "WEB-INF/templates";
	static String resultTemplateName = "ViewMyAuctions-Result.ftl";
	...
	public void doGet( HttpServletRequest req, HttpServletResponse res ) throws ServletException, IOException {
        ...
        try {
			auctionObj = logic.viewMyAuctions( person.getName() );
        ...
        for( int i = 0; i < auctionObj.size(); i++ ) {
        	...
        	try {
				...
				DTError.error(cfg, toClient, "ViewMyAuctions: Could not get item name " + e1);
			}
        	auctionList.add( auction.getMinPrice() );
        	try {
				auctionList.add( auction.getSellingPrice() );
			} catch (DTException e) {
				DTError.error(cfg, toClient, "ViewMyAuctions: Could not get selling price of auction " + e);
				return;
			}
